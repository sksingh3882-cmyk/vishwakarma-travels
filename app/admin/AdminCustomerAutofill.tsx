...
function setInput(el: HTMLInputElement | null, val: string) { if (!el) return; el.value = val; el.dispatchEvent(new Event("input", { bubbles: true })); el.dispatchEvent(new Event("change", { bubbles: true })); }
function setSelect(options: string[], val: string) { const s = Array.from(document.querySelectorAll<HTMLSelectElement>(".admin-shell form select")).find((el) => options.some((o) => Array.from(el.options).some((x) => x.text.toLowerCase() === o.toLowerCase()))); if (!s) return; s.value = val; s.dispatchEvent(new Event("change", { bubbles: true })); }
...
function fill(r: Row) { setInput(input("whatsapp"), mobile(r.mobile || r.phone)); setInput(input("pickup"), clean(r.address)); setInput(input("drop"), clean(r.drop_location)); setInput(document.querySelector<HTMLInputElement>('.admin-shell form input[type="date"]'), clean(r.journey_date)); setInput(input("time"), clean(r.journey_time)); setSelect(["Sedan", "SUV", "Mini Passenger Bus"], clean(r.vehicle_type)); setSelect(["Desire", "Ertiga", "Innova", "Force Traveller"], clean(r.vehicle_model)); setSelect(["One Way Drop Pickup", "Local Movement", "Local Movment", "Outstation Movement", "Outstation Movment"], clean(r.service)); }
function clearAuto() { setInput(input("pickup"), ""); setInput(input("drop"), ""); setInput(document.querySelector<HTMLInputElement>('.admin-shell form input[type="date"]'), ""); setInput(input("time"), ""); setSelect(["Sedan", "SUV", "Mini Passenger Bus"], "Sedan"); setSelect(["Desire", "Ertiga", "Innova", "Force Traveller"], "Desire"); setSelect(["One Way Drop Pickup", "Local Movement", "Local Movment", "Outstation Movement", "Outstation Movment"], "One Way Drop Pickup"); }
