...
export default function AdminCustomerAutofill() {
  ...
  useEffect(() => {
    ...
    const run = () => setTimeout(fill, 600);
    load().then(run); const t = setInterval(() => load().then(fill), 5000); document.addEventListener("blur", run, true); document.addEventListener("mousedown", run, true); document.addEventListener("touchstart", run, true); return () => { clearInterval(t); document.removeEventListener("blur", run, true); document.removeEventListener("mousedown", run, true); document.removeEventListener("touchstart", run, true); };
  }, []);
  return null;
}
