...
export default function AdminPage() {
    ...
    async function loadData() {
      ...
      try {
        const [customerRes, vehicleRes, bookingRes] = await Promise.all([
          ...
          fetch(`${supabaseUrl}/rest/v1/bookings?select=*&order=created_at.desc`, { headers }),
        ]);
  ...
  function fillCustomer(value: string) {
    ...
    const customer = customers.find((item) => {
      ...
      return name.includes(search) || mobile.includes(search) || address.includes(search);
    });
...
+91 7667989203`;
  ...
  async function saveBooking(bookingId: string) {
    ...
    if (!response.ok) {
      ...
      console.log("Supabase booking save error:", errorText);
      return false;
    }
  ...
  async function saveCustomerIfNew() {
    ...
    const alreadyExists = customers.some((item) => cleanPhone(item.mobile || "") === phone);
