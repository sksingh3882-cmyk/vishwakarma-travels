...
import { confirmBookingRequestAfterDownload, ensureBookingRequestDriverAssignmentCode, fetchBookingRequestById, type BookingRequestRecord } from "@/lib/bookingRequestService";
...
async function driverTripLink() {
  if (!activeBookingRequest?.id) return "";

  const record = await ensureBookingRequestDriverAssignmentCode({
    supabaseUrl,
    supabaseKey,
    requestId: activeBookingRequest.id,
  });

  return `${window.location.origin}/d/${encodeURIComponent(record.driverAssignmentCode || "")}`;
}
async function openDriverWhatsAppFromForm(){if(!activeBookingRequest?.id)return alert("Pehle customer booking request accept/select karo.");window.location.href=`https://api.whatsapp.com/send?phone=91${cleanPhone(form.driverMobile)}&text=${encodeURIComponent(`🚕 *Vishwakarma Travels - Trip Assigned*\n\nHello *${form.driverName||"Driver"}*,\n\nYour trip has been assigned.\n\n👤 *Customer:* ${form.customerName||"-"}\n📞 *Customer Mobile:* +91 ${cleanPhone(form.customerPhone)||"-"}\n📍 *Pickup:* ${form.pickup||"-"}\n📍 *Drop:* ${form.drop||"-"}\n📅 *Date:* ${formatDate(form.journeyDate)||"-"}\n🕒 *Time:* ${formatTime(form.journeyTime)||"-"}\n🚖 *Vehicle:* ${form.vehicleType||"-"} ${form.vehicleModel||"-"}\n🔢 *Vehicle No:* ${vehicleNo(form.vehicleNumber)||"-"}\n\n👇 Tap this link to view trip details:\n${await driverTripLink()}\n\nPlease call the customer before starting the trip.\n\n- Vishwakarma Travels`)}`}
  ...
  try {
    ...
    await openDriverWhatsAppFromForm();
