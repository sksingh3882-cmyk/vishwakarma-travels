...
<SecureBookingPopup />
  ...
  <section style={wrap}><form id="booking-form" onClick={e=>e.stopPropagation()} onSubmit={submit} style={card}><h2 style={title}>Book Your Ride</h2><p>For Booking Please Fill The Details Below.</p><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,gridColumn:"1/-1",position:"relative",zIndex:500}}><Drop kind="service" label="Select Service" options={serviceOptions} value={form.service}/><Drop kind="vehicle" label="Select Vehicle" options={vehicleOptions} value={form.vehicle}/></div><div style={{position:"relative"}}><input placeholder="Your Name" value={form.name} onFocus={()=>setShowSug(true)} onChange={e=>{set("name",e.target.value);setShowSug(true)}} onBlur={()=>setTimeout(()=>{find();setShowSug(false)},180)} style={inp} required/>{showSug&&suggestions.length>0&&<div style={sbox}>{suggestions.map((c,i)=><button key={i} type="button" onMouseDown={()=>apply(c)} style={sitem}><b>{c.name}</b><span>{cleanPhone(c.mobile||c.phone||"")}</span><small>{c.address}</small></button>)}</div>}</div><input placeholder="Mobile Number" value={form.mobile} onChange={e=>set("mobile",cleanPhone(e.target.value))} onBlur={find} style={inp} required/><input placeholder="Pickup Location" value={form.pickup} onChange={e=>set("pickup",e.target.value)} style={inp} required/><input placeholder="Drop Location" value={form.drop} onChange={e=>set("drop",e.target.value)} style={inp} required/>{oldDrops.length>0&&<div style={chips}><b>Old Drop Locations</b><div>{oldDrops.map(d=><button type="button" key={d} onClick={()=>set("drop",d)} style={chip}>{d}</button>)}</div></div>}<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,gridColumn:"1/-1",position:"relative",zIndex:10}}><DatePick/><TimePick/></div><div style={{gridColumn:"1/-1"}}>
