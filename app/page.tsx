...
export default function Home(){
 ...
 function downloadCopy(){const d=data();if(!d.name||!d.mobile||!d.pickup||!d.drop)return alert("Booking copy ke liye name, mobile, pickup aur drop fill karo.");const c=document.createElement("canvas");c.width=1080;c.height=1920;const x=c.getContext("2d");if(!x)return;const wrap=(t:string,xx:number,y:number,w:number,lh:number)=>{let line="";for(const word of String(t||"-").split(" ")){const test=line+word+" ";if(x.measureText(test).width>w&&line){x.fillText(line.trim(),xx,y);line=word+" ";y+=lh}else line=test}x.fillText(line.trim(),xx,y);return y};const row=(l:string,v:string,y:number)=>{x.fillStyle="#0b2d6b";x.font="bold 38px 'Times New Roman', Times, serif";x.fillText(l,95,y);x.fillStyle="#111";x.font="38px 'Times New Roman', Times, serif";const ly=wrap(v,430,y,540,43);return Math.max(y+58,ly+18)};const rr=(a:number,b:number,w:number,h:number,r:number)=>{x.beginPath();x.roundRect(a,b,w,h,r);x.fill()};x.fillStyle="#f4f7fb";x.fillRect(0,0,1080,1920);x.fillStyle="#fff";rr(18,18,1044,1884,28);const draw=()=>{x.fillStyle="#fff7ed";rr(60,560,230,58,25);x.fillStyle="#ea580c";x.font="bold 34px 'Times New Roman', Times, serif";x.fillText("Booking Copy",86,599);x.fillStyle="#0b2d6b";x.font="bold 58px 'Times New Roman', Times, serif";x.fillText("Vishwakarma Travels",78,700);x.fillStyle="#64748b";x.font="35px 'Times New Roman', Times, serif";x.fillText("Your ride booking details",80,748);let y=840;y=row("Customer Name",d.name,y);y=row("Mobile",d.mobile,y);y=row("Service",d.service,y);y=row("Vehicle",d.vehicle,y);y=row("Pickup",d.pickup,y);y=row("Drop",d.drop,y);y=row("Date",fmt(d.bookingDate),y);y=row("Time",d.bookingTime,y);const dy=Math.max(y+50,1360);x.fillStyle="#087a31";x.font="bold 36px 'Times New Roman', Times, serif";x.fillText("Declaration",95,dy);x.strokeStyle="#087a31";x.lineWidth=6;x.beginPath();x.moveTo(75,dy+28);x.lineTo(1005,dy+28);x.stroke();x.fillStyle="#111";x.font="34px 'Times New Roman', Times, serif";let yy=dy+85;for(const t of ["Book A Cab Atleast 24 Hour Before Travelling Otherwise Booking May Not Be Confirmed","After the booking is Confirmed , Customer will have to make the Advance Payment","Rs.500 Cancellation Charge will have to be paid on Cancellation of Booking under any Circumtances"]){x.fillText("*",100,yy);yy=wrap(t,145,yy,830,43)+55}x.strokeStyle="#111";x.lineWidth=4;x.setLineDash([22,14]);x.beginPath();x.moveTo(95,1768);x.lineTo(985,1768);x.stroke();x.setLineDash([]);x.fillStyle="#0b2d6b";x.textAlign="center";x.font="bold 35px 'Times New Roman', Times, serif";x.fillText("Thank You And Wish You A Very Happy Journey",540,1822);x.font="bold 42px 'Times New Roman', Times, serif";x.fillText("Vishwakarma Travels",540,1872);const a=document.createElement("a");a.href=c.toDataURL("image/jpeg",.95);a.download=`Vishwakarma-Booking-${Date.now()}.jpg`;a.click();setNotice(true);setTimeout(()=>setNotice(false),3500)};const img=new Image();img.crossOrigin="anonymous";img.onload=()=>{x.drawImage(img,42,42,996,480);draw()};img.onerror=draw;img.src="/cars/popup_banner.png"}
 return <main style={page}>{pending&&<div style={overlay}><div style={modal}><button onClick={()=>setPending(null)} style={close}>×</button><img src="/cars/popup_banner.png" style={banner}/><div style={{padding:14}}><h2 style={{color:"#0b2d6b",margin:0}}>Confirm Your Ride Details</h2><p>Please check details before WhatsApp submit.</p><div style={grid}>{["name","mobile","service","vehicle","bookingDate","bookingTime"].map(k=><div style={info} key={k}><b>{k}</b><span>{pending[k]}</span></div>)}</div><div style={route}><b>Pickup</b><span>{pending.pickup}</span><b>Drop</b><span>{pending.drop}</span></div><div style={actions}><button onClick={()=>setPending(null)} style={cancel}>Cancel</button><button onClick={send} style={confirm}>Confirm & Submit</button></div></div></div></div>}
