...
const [showCover,setShowCover]=useState(true);
 ...
 const [form,setForm]=useState<F>(empty),[errors,setErrors]=useState<FormErrors>({}),[customers,setCustomers]=useState<any[]>([]),[bookings,setBookings]=useState<any[]>([]),[showSug,setShowSug]=useState(false),[showOldPickups,setShowOldPickups]=useState(true),[showOldDrops,setShowOldDrops]=useState(true),[pending,setPending]=useState<any>(null),[notice,setNotice]=useState(false),[openDrop,setOpenDrop]=useState<"service"|"vehicle"|"date"|"time"|null>(null),[calMonth,setCalMonth]=useState(()=>new Date()),[customTime,setCustomTime]=useState({h:"07",m:"40",ap:"AM"});
 ...
 const apply=(c:any)=>{setForm(p=>({...p,name:c.name||p.name,mobile:cleanPhone(c.mobile||c.phone||p.mobile)}));setShowSug(false)};
...
const oldPickups=Array.from(new Set(bookings.filter(b=>{const bp=cleanPhone(b.customer_phone||b.mobile||b.customer_mobile||"");const bn=String(b.customer_name||b.name||"").toLowerCase();return(bp&&bp===cleanPhone(form.mobile))||(form.name&&bn.includes(form.name.toLowerCase()))}).map(b=>b.pickup||b.address||"").filter(Boolean))).slice(0,4);
...
<SecureBookingPopup />
  ...
  </div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,gridColumn:"1/-1"}}><Drop kind="service" label="Select Service" options={serviceOptions} value={form.service}/><Drop kind="vehicle" label="Select Vehicle" options={vehicleOptions} value={form.vehicle}/></div><div style={{position:"relative"}}><input placeholder="Your Name" value={form.name} onFocus={()=>setShowSug(true)} onChange={e=>{set("name",e.target.value);setShowSug(true);setShowOldPickups(true);setShowOldDrops(true)}} onBlur={()=>setTimeout(()=>{find();setShowSug(false)},180)} style={inp} required/>{showSug&&suggestions.length>0&&<div style={sbox}>{suggestions.map((c,i)=><button key={i} type="button" onMouseDown={()=>apply(c)} style={sitem}><b>{c.name}</b><span>{cleanPhone(c.mobile||c.phone||"")}</span><small>{c.address}</small></button>)}</div>}</div><input placeholder="Mobile Number" value={form.mobile} onChange={e=>{set("mobile",cleanPhone(e.target.value));setShowOldPickups(true);setShowOldDrops(true)}} onBlur={find} style={inp} required/><input placeholder="Pickup Location" value={form.pickup} onChange={e=>set("pickup",e.target.value)} style={inp} required/>{showOldPickups&&oldPickups.length>0&&<div style={chips}><b>Old Pickup Locations</b><div>{oldPickups.map(p=><button type="button" key={p} onClick={()=>{set("pickup",p);setShowOldPickups(false)}} style={chip}>{p}</button>)}</div></div>}<input placeholder="Drop Location" value={form.drop} onChange={e=>set("drop",e.target.value)} style={inp} required/>{showOldDrops&&oldDrops.length>0&&<div style={chips}><b>Old Drop Locations</b><div>{oldDrops.map(d=><button type="button" key={d} onClick={()=>{set("drop",d);setShowOldDrops(false)}} style={chip}>{d}</button>)}</div></div>}<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,gridColumn:"1/-1"}}><DatePick/><TimePick/></div><div style={{gridColumn:"1/-1"}}>
