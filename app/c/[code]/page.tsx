...
import {
  fetchBookingRequestByConfirmationCode,
  fetchBookingRequestById,
  type BookingRequestInput,
  type BookingRequestRecord,
} from "@/lib/bookingRequestService";

export default function ShortBookingConfirmationPage() {
  ...
  useEffect(() => {
    ...
    async function loadBooking() {
      ...
      try {
        let latest = await fetchBookingRequestByConfirmationCode({
          supabaseUrl,
          supabaseKey,
          confirmationCode: code,
        });

        if (!latest && code.includes("-")) {
          latest = await fetchBookingRequestById({
            supabaseUrl,
            supabaseKey,
            requestId: code,
          });
        ...
        console.log("Short booking confirmation link failed:", err);
  ...
  return (
    <main style={page}>
      {loading ? (
        <div style={box}>
          <h2 style={title}>Opening Booking Confirmation...</h2>
